...
PRE_PUSH_HOOK_AUTH="GIT_PERSONAL_ACCESS_TOKEN"
if [ ! -f $GH_SETUP ]; then
    ############ Get username from remote url ##############
    if [[ (-z "$GITHUB_USER_LOGIN") && ("$PRE_PUSH_HOOK_AUTH" == "NONE") ]]; then
    ...
    if [ -z "$GITHUB_USER_LOGIN" ]; then
        ...
        elif [[ "$PRE_PUSH_HOOK_AUTH" == "LOCAL_HOOK_TOKEN" ]]; then
            GITHUB_USER_LOGIN=$(git gui--askpass "Please enter your Local hook token:")
        else
